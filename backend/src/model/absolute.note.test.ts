import {absoluteNoteFactory} from "./absolute.note";
import {keyFactory} from "./key";
import {intervalFactory} from "./interval";

describe('absolute note', () => {
    it('instantiates middle c', () => {
        const actual = absoluteNoteFactory.middleC;

        expect(actual.getAbc(keyFactory.cMajor)).toBe('C')
    })

    it('adds major and perfect intervals', () => {
        const intervals = [
            intervalFactory.perfect(1),
            intervalFactory.major(2),
            intervalFactory.major(3),
            intervalFactory.perfect(4),
            intervalFactory.perfect(5),
            intervalFactory.major(6),
            intervalFactory.major(7),
            intervalFactory.perfect(8)
        ]
    })
    /*
    @Test
    ...
    @Test
    fun subtractsCompoundIntervalsCorrectly() {
        val interval = Interval.major(9)
        val expected = AbsoluteNote(Note.b.flatten, Octave.default.down.down)

        val actual = AbsoluteNote.middleC - interval
