...
class OctaveTests {
  ...
  def when_getAbcInHigherOctaves_then_correctResponse(octavesHigher: Int): Unit = {
    val octave = List.range(0, octavesHigher).foldRight(Octave.DEFAULT)((_, it) => it.getUp)
    val note = Note.E
  ...
  def when_getAbcInLowerOctaves_then_correctResponse(octavesLower: Int): Unit = {
    val octave = List.range(0, octavesLower).foldRight(Octave.DEFAULT)((_, it) => it.getDown)
    val note = Note.G
